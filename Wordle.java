public class Wordle {
    ...
    public static String chooseSecretWord(String[] dict) {
		int rnd = (int)(Math.random() * (double)dict.length);
        return dict[rnd];
    }
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
		for(int i = 0; i < guess.length(); i ++) {
            if(containsChar(secret,guess.charAt(i))) {
                if(secret.charAt(i) == guess.charAt(i)) { // These means the character's are both at the same spot
                    resultRow[i] = 'G';
                }
                else { // That means the character is not in the same place as the letter at it is in the secret word
                    resultRow[i] = 'Y';
                }
            }
            else { // That means the character does not exist in the secret word
                resultRow[i] = '_';
            }
        }
    }
    ...
    public static boolean isAllGreen(char[] resultRow) {
        for(int i = 0; i < resultRow.length; i++) {
            if(resultRow[i] != 'G') { // If a single char is not G then we are not all green
                return false;
            }
        }
        return true; // If we finished going through the entire array and it was all G than return true
    }
    ...
    public static void main(String[] args) {
        ...
        if (!won) {
            ...
            System.out.println("The seret word was: " + secret.toUpperCase());
